    ...
    public class HomeController : Controller
    {
        ...
        public async Task<IActionResult> GetMessages(int id)
        {
            if (_signInManager.IsSignedIn(User))
            {
                ...
                List<MessageVM> responseMess = new List<MessageVM>();
                List<Users> uniqueUser = new List<Users>();

                foreach (var msg in messages)
                {
                    var user = db.Users.FirstOrDefault(u => u.Id == msg.SenderId);
                ...
                {
                    int msgCount = messages.Where(m => m.SenderId == user.Id && m.Status == false).Count();
                    ...
                    var response = new MessageVM()
                    {
                        ...
                        SenderId = user.Id,
                        Message = messages.LastOrDefault(m => m.SenderId == user.Id).Message,
                        Count = msgCount,
                        Photo = db.Profile.FirstOrDefault(p => p.UsersId == user.Id).Photo,
                        Date = messages.LastOrDefault(m => m.SenderId == user.Id).Date.ToString("yyyy-MM-dd HH:mm:ss")
                    };

                    responseMess.Add(response);
                }

                int unreadCount = db.Messages.Where(m => m.GetterId == id && m.Status == false).Count();

                return Json(new { responseMess, unreadCount });
            }
