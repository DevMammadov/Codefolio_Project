            ...
            $.ajax({
                ...
                success: function (result) {
                    ...
                    for (let item of result.notifies) {
                        ...
                        if (item.type == "user") { folder = "Users/"; text = item.text; Nport = ''; href = "/Profile/About/"+item.senderProfileId; }
        ...
        setInterval(function () {
            $.ajax({
                ...
                success: function (result) {
                    ...
                    if (result.unreadCount > 0) {
                        notifyBadge.show().html(result.unreadCount);
                    }
                    else {
                        notifyBadge.hide()
                    }
                    ...
                    notifyList.empty();
                    for (let msg of result.responseMess) {

                        if (msg.message.length > 34) { msg.message = msg.message.substr(0, 34) }

                        let hide = "";
                        if (msg.count == 0) { hide = "d-none"; }

                        let message = '<li> <a href = "/Message/Index/' + msg.senderId + '" > <img src="' + src + folder + msg.photo + '" alt="">' +
                            ' <div> <h6>' + msg.name + ' ' + msg.surname + '</h6>' +
                            '<div class="msg">' + msg.message + '<span class="badge badge-danger ' + hide +' p-1">'+ msg.count +'</span> </div>' +
                            '<small>' + calculateDate(msg.date) +'</small>' +
                            '</div> </a> </li>';
                            notifyList.prepend(message);
                    }
                }
            });
       }, 2000);
